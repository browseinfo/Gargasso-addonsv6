...
#        (_check_product, 'BoM line product should not be same as BoM product.', ['product_id'])
    ...
    def _bom_explode_new(self, cr, uid, bom, factor, mo_prod_seq=0, properties=[], addthis=False, level=0):
        """ Finds Products and Workcenters for related BoM for manufacturing order.
        @param bom: BoM of particular product.
        @param factor: Factor of product UoM.
        @param properties: A List of properties Ids.
        ...
        if bom.type == 'phantom' and not bom.bom_lines:
            newbom = self._bom_find(cr, uid, bom.product_id.id, bom.product_uom.id, properties)
            if newbom:
                res = self._bom_explode_new(cr, uid, self.browse(cr, uid, [newbom])[0], factor*bom.product_qty, mo_prod_seq, properties, addthis=True, level=level+10)
                result = result + res[0]
                result2 = result2 + res[1]
                phantom = True
            else:
        ...
        if not phantom:
            ...
            for bom2 in bom.bom_lines:
                if not mo_prod_seq:
                    res = self._bom_explode_new(cr, uid, bom2, factor, bom2.x_mo_prod_seq, properties, addthis=True, level=level+10)
                    mo_prod_seq = 0
                else:
                    res = self._bom_explode_new(cr, uid, bom2, factor, mo_prod_seq, properties, addthis=True, level=level+10)
                result = result + res[0]
                result2 = result2 + res[1]
        return result, result2
...
#    _order = 'priority desc, date_planned asc';
    ...
    def action_compute(self, cr, uid, ids, properties=[]):
        ...
        for production in self.browse(cr, uid, ids):
            ...
            res = bom_obj._bom_explode_new(cr, uid, bom_point, factor / bom_point.product_qty, 0, properties)
