...
class stock_move_spool(report_sxw.rml_parse):
    ...
    def _get_spool_stock_move(self, move):
        res = []
       
        for stock in self.pool.get('stock.move').browse(self.cr, self.uid, move['stock_move_ids']):
            date1 = []
            ...
            if stock.partner_id:
                ...
                if stock.sale_line_id:
                    ...
                    crm_po_no = self.pool.get('sale.order').browse(self.cr,self.uid,so_id).crm_po_no
            res.append({
