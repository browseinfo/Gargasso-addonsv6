...
class stock_production_lot(osv.osv):
    ...
    def _check_unique_prodlot_name(self, cr, uid, ids, context=None):
        """ Checks whether production lot name already exists.
        @return: True or False
        """

        flag=True
        prodlot_obj = self.pool.get('stock.production.lot')
        prodlots = prodlot_obj.browse(cr, uid, ids, context=context)
        for lot in prodlots:
            print lot.name
            prodlot_id = prodlot_obj.search(cr, uid, [('name', '=', lot.name),('id', '!=', lot.id)])
            if prodlot_id:
                flag=False
        return flag
    ...
    _constraints = [
        (_check_unique_prodlot_name,
            'Production Lot already exists, you must assign a unique production lot name',
            ['name'])]
...
#     def _check_available_real_qty(self, cr, uid, ids, context=None):
#         """ Checks whether move is done or not and production lot is assigned to that move.
#         @return: True or False
#         """
# #        print move.type
...
#         for move in self.browse(cr, uid, ids, context=context):
#             check_qty = 0
#             print '_PPPPPPPPPPP________',move.product_id.name
#             bom_search = bom_obj.search(cr, uid, [('product_id','=',move.product_id.id)])
#             print '________bom_search__________',bom_search
#             if bom_search:
#                 bom_browse = bom_obj.browse(cr, uid, bom_search[0])
#                 print '__________-bom_browse.type________',bom_browse.product_qty,bom_browse.type,bom_browse.product_id.name
#                 if bom_browse.type == 'phantom':
#                     raise osv.except_osv(_('Error!'), _('In BOM of product %s type is Sets/Phantom is given')%bom_browse.product_id.name)
#                 else:
#                     if move.prodlot_id and move.picking_id.type=='out' and move.state<>'done':
#                         product_uom = product_obj.browse(cr, uid, move.product_id.id, context=context).uom_id
#                         prodlotid = prodlot.browse(cr, uid, move.prodlot_id.id, context=context)
...
#                         for move1 in self.browse(cr, uid, ids, context=context):
#                             if move.prodlot_id.id == move1.prodlot_id.id:
#                                 check_qty += move1.product_qty
#                         print '________-check_qty_______',check_qty
#                         print '_________amount_actual__________',amount_actual
#                         if Decimal("%.15g" % round(check_qty,2)) > Decimal("%.15g" % amount_actual):
#                             return False
#                         if Decimal("%.15g" % round(move.product_qty,2)) > Decimal("%.15g" % amount_actual):
#                             return False
#                         
#         return True
...
#        print move.type
    ...
    def _check_spool_assignment(self, cr, uid, ids, context=None):
        """ Checks whether spool is assigned to single box or not.
        @return: True or False
        """
        for move in self.browse(cr, uid, ids, context=context):
            if move.x_spool_number and move.picking_id.type=='out' and move.state<>'done' and move.tracking_id:
                all_moves = self.search(cr, uid, [('x_spool_number','=',move.x_spool_number),('picking_id','=',move.picking_id.id)])
                tracking_id = move.tracking_id.id
                if len(all_moves)>0:
                    for item in self.browse(cr, uid, all_moves, context=context):
                        if item.tracking_id.id != tracking_id:
                            return False        
        return True
...
#        'move_value_sale'  :  fields.float('Move Value Sale', digits_compute=dp.get_precision('Account') ),
    ...
    _constraints = [
        ...
        (_check_product_lot,
            ...
            ['prodlot_id']),
        (_check_available_real_qty,
            'Assigned production lot does not have enough quantity.',
            ['prodlot_id']),
        (_check_spool_assignment,
            'Same spool number cannot be assigned to different boxes.',
            ['x_spool_number'])]
...
#        product_uom = product_obj.browse(cr, uid, product_id, context=ctx).uom_id
        product_uom_categ = product_obj.browse(cr, uid, product_id, context=ctx).uom_id.category_id.id
        product_ref_uom_id = uom_obj.search(cr, uid, [('category_id', '=', product_uom_categ), ('uom_type', '=', 'reference')], context=ctx)[0]
        product_ref_uom = uom_obj.browse(cr, uid, product_ref_uom_id, context=ctx)
        ...
        amount_actual = uom_obj._compute_qty_obj(cr, uid, product_ref_uom, prodlot.stock_available, uom, context=ctx)
        warning = {}
