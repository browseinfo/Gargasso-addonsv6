...
class product_product(osv.osv):
    sys.path.append('/home/erp/garg/server/bin/addons/product/vtwsclib/Vtiger')
    ...
    _columns = {
        ...
        'transfertovtiger': fields.boolean('f'),
        'vtigerid': fields.integer('VtigerID',help='Gives the record id of vtiger database.')
    }
    
    def transfertovtiger(self, cr, uid, ids, context=None):
        from WSClient import *
        client = Vtiger_WSClient('http://184.73.234.224/vtigerCRM/')
        recordId=0
        for product in self.browse(cr, uid, ids, context=None):
            print product.transfertovtiger
            print ids
            try:
                if(not product.transfertovtiger):
                    login = client.doLogin('admin', 'DxSVd99g530OKs9')
                    if not login:
                        raise osv.except_osv(_('valid action !'), _('Unable to login CRM, please contact admin.'))
                    else:
                        recordInfo = client.doCreate('Products',{ 'productname' : product.name_template, 'productcode' : product.default_code,'discontinued':'1','productcategory' : 'PTFE Insulated Wire & Cabels (CETSH-85441920)', 'glacct' : '300-Sales-Multicore', 'unit_price' : '0', 'usageunit' : 'M', 'currency_id' : '2' })
                        recordId = client.getRecordId(recordInfo['id'])
                        self.write(cr, uid, ids, {'transfertovtiger': 't','vtigerid': recordId})
                else:
                    raise osv.except_osv(_('valid action !'), _('Product already transfered to CRM!'))
            except IOError:
                raise osv.except_osv(_('valid action !'), _('Unable to connect to CRM, please contact admin.'))
        return recordId
    ...
    def name_get(self, cr, user, ids, context=None):
        ...
        for product in self.browse(cr, user, ids, context=context):
            check_product = self.pool.get('product.product').read(cr, user, product.id, context)
            if check_product:
