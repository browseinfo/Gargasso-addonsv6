...
class change_production_qty(osv.osv_memory):
    ...
    def change_prod_qty(self, cr, uid, ids, context=None):
        ...
        date_format = "%d/%m/%Y"
        check_date = datetime.strptime('27/08/2013', date_format)
        prod_date = prod_obj.browse(cr, uid, record_id, context=context)
        create_date = prod_date.create_date
        create_date = create_date.split(' ')[0].split('-')
        prod_date = datetime.strptime(create_date[2]+'/'+create_date[1]+'/'+create_date[0],date_format)
        if prod_date >= check_date:
            for wiz_qty in self.browse(cr, uid, ids, context=context):
                ...
                res = bom_obj._bom_explode_new(cr, uid, bom_point, factor / bom_point.product_qty, 0, [])
            ...
            raise osv.except_osv(_('Error'), _("Unable to change quantity. Production order create date should be greater than '27/08/2013'."))
