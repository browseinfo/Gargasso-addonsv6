...
class mrp_production(osv.osv):
	...
	def action_produce(self, cr, uid, production_id, production_qty, production_mode, context=None):
		...
		if production_mode in ['consume', 'consume_produce']:
				...
				if raw_products:
					for raw_product in raw_products:
						...
						if raw_product.prodlot_id:
							...
							if grouped_product_qty.has_key(raw_product.prodlot_id.id):
								grouped_product_qty[raw_product.prodlot_id.id] = grouped_product_qty[raw_product.prodlot_id.id] + qty 
							else:
								grouped_product_qty[raw_product.prodlot_id.id] = qty
							if not grouped_product.has_key(raw_product.prodlot_id.id):
								grouped_product[raw_product.prodlot_id.id] = raw_product.product_id
							if not grouped_prodlot.has_key(raw_product.prodlot_id.id):
								grouped_prodlot[raw_product.prodlot_id.id] = raw_product.prodlot_id
						else:
						...
						avail_prodlot_qty[raw_product.prodlot_id.id] = amount_actual
						prodlot_uom[raw_product.prodlot_id.id] = uom
			...
			if error_message == '':
				for element in grouped_product_qty:
					
					if grouped_product[element].categ_id.parent_id.name == 'Tapes' or grouped_product[element].categ_id.parent_id.name == 'Conductors':
						if prodlot_uom[element].uom_type == 'reference':
							if avail_prodlot_qty[element] - grouped_product_qty[element] < -1:
								if error_message == '':
									error_message = 'Production lot "' + grouped_prodlot[element].name + '" assigned to "' + grouped_product[element].name + '" does not have enough quantity.'
								else:
									error_message = error_message + ' Production lot "' + grouped_prodlot[element].name + '" assigned to "' + grouped_product[element].name + '" does not have enough quantity.'
						elif prodlot_uom[element].uom_type == 'smaller':
							if avail_prodlot_qty[element]/uom.factor - grouped_product_qty[element]/uom.factor < -1:
								if error_message == '':
									error_message = 'Production lot "' + grouped_prodlot[element].name + '" assigned to "' + grouped_product[element].name + '" does not have enough quantity.'
								else:
									error_message = error_message + ' Production lot "' + grouped_prodlot[element].name + '" assigned to "' + grouped_product[element].name + '" does not have enough quantity.'
						elif prodlot_uom[element].uom_type == 'bigger':
							if avail_prodlot_qty[element]*uom.factor - grouped_product_qty[element]*uom.factor < -1:
								if error_message == '':
									error_message = 'Production lot "' + grouped_prodlot[element].name + '" assigned to "' + grouped_product[element].name + '" does not have enough quantity.'
								else:
						...
						if grouped_product_qty[element] > avail_prodlot_qty[element]:
							if error_message == '':
								error_message = 'Production lot "' + grouped_prodlot[element].name + '" assigned to "' + grouped_product[element].name + '" does not have enough quantity.'
							else:
								error_message = error_message + ' Production lot "' + grouped_prodlot[element].name + '" assigned to "' + grouped_product[element].name + '" does not have enough quantity.'
