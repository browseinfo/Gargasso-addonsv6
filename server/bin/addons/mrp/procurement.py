...
class procurement_order(osv.osv):
    ...
    def make_mo(self, cr, uid, ids, context=None):
        ...
        for procurement in procurement_obj.browse(cr, uid, ids, context=context):
            ...
            mo_type='Export'
            if procurement.move_id.address_id:
            	motype=1
            	if procurement.move_id.address_id.country_id.name == 'India':
            		produce_id = production_obj.create(cr, uid, {
																'origin': procurement.origin,
																'name': self._get_mo_name(cr, uid, procurement.product_id.id),
                ...
                production_obj.set_order_type(cr, uid, [produce_id])
            
            res[procurement.id] = produce_id
            self.write(cr, uid, [procurement.id], {'state': 'running'})
            prod_property = production_obj.browse(cr, uid, produce_id, context=context)
            mrp_property_obj = self.pool.get('mrp.property')
            mrp_property_id = mrp_property_obj.search(cr, uid, [('name','=',prod_property.x_job_order_type)])
            
            bom_result = production_obj.action_compute(cr, uid, [produce_id], mrp_property_id)
...
#                    [produce_id], properties=[x.id for x in procurement.property_ids])
    ...
    def _get_mo_name(self, cr, uid, product_id):
