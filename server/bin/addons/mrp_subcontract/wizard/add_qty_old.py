    ...
    def add_prod_qty(self, cr, uid, ids, context=None):
		...
		for wiz_qty in self.browse(cr, uid, ids, context=context):
			...
			for move in move_lines:
				...
				list_new = []
				for line_prod in least.bom_lines:
					if move.product_id.id == line_prod.product_id.id:
						ch_qty = line_prod.product_qty * wiz_qty.product_qty / bom_point.product_qty
						for r in res[0]:
							if r['product_id'] == line_prod.product_id.id:
								change_qty = move.product_qty + ch_qty
								move_lines_obj.write(cr, uid, [move.id], {'product_qty' :  change_qty})
				...
				if m.product_id.id == prod.product_id.id:
					...
					for sub_product_line in prod.bom_id.sub_products:
						if sub_product_line.subproduct_type == 'variable':
							new_qty = move_browse.product_qty + (wiz_qty.product_qty * sub_product_line.product_qty / bom_point.product_qty)
							move_lines_obj.write(cr, uid, [m.id], {'product_qty': new_qty})
