...
class stock_move(osv.osv):
    ...
    def _check_available_real_qty(self, cr, uid, ids, context=None):
        """ Checks whether move is done or not and production lot have enough quantity.
        @return: True or False
        """
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if move.prodlot_id and move.picking_id.type=='out' and move.state<>'done':
                ...
                if move.product_qty > amount_actual:
                    return False
        return True
...
#        'move_value_sale'  :  fields.float('Move Value Sale', digits_compute=dp.get_precision('Account') ),
    ...
    _constraints = [
        ...
        (_check_available_real_qty,
            'Selected production lot does not have enough quantity.',
            ['prodlot_id'])]
