...
#        'name': lambda x, y, z, c: x.pool.get('ir.sequence').get(y, z, 'mrp.production') or '/',
    ...
    def product_id_change(self, cr, uid, ids, product_id,mo_name, context=None):
        ...
        print product.product_tmpl_id.categ_id.parent_id.name
        print product.product_tmpl_id.categ_id.parent_id
        if product.product_tmpl_id.categ_id.parent_id.name=='Tapes':
            if mo_name==False or mo_name[:2]=='MO':
                result = {
            ...
            if mo_name==False or mo_name[:2]=='JR':
                result = {
                          'product_uom': product.uom_id and product.uom_id.id or False,
                          'bom_id': bom_id,
                          'routing_id': routing_id,
...
#            for line in sortedRes:
#                if (line['name'][:5] <> 'Jumbo'):
#                    curId = sortedRes[0]['product_id']
#                    break
            
            for line in prodRes:
                if (curId <> line['product_id']):
                    #line['product_qty'] = intSum
                    prodRes[it - 1]['product_qty'] = intSum
                    #print "Sum of ", curId, " = ", intSum
                    groupedRes.append(prodRes[it - 1])
                    curId = line['product_id']
                    intSum = 0                           
                intSum = intSum + line['product_qty']       
                if (it == len(prodRes) - 1):
                   #print it, len(sortedRes), sortedRes[it]['product_id']
                   prodRes[it]['product_qty'] = intSum
                   groupedRes.append(prodRes[it])
                #print line['product_id'],' - ', line['product_qty'], 'it=', it
                it = it + 1
            ##############################################################
#            for line in sortedRes:
#                    if (curId <> line['product_id']):
#                        #line['product_qty'] = intSum
#                        sortedRes[it - 1]['product_qty'] = intSum
#                        #print "Sum of ", curId, " = ", intSum
#                        groupedRes.append(sortedRes[it - 1])
#                        curId = line['product_id']
#                        intSum = 0                           
#                    intSum = intSum + line['product_qty']       
#                    if (it == len(sortedRes) - 1):
#                       #print it, len(sortedRes), sortedRes[it]['product_id']
#                       sortedRes[it]['product_qty'] = intSum
#                       groupedRes.append(sortedRes[it])
#                    #print line['product_id'],' - ', line['product_qty'], 'it=', it
#                    it = it + 1
            ##############################################################
            ...
            #for line in groupedRes:
                #print line['product_id'], line['product_qty']
#            results = res[0]
            results = groupedRes
            results2 = res[1]
            for line in results:
    ...
    def action_confirm(self, cr, uid, ids):
        ...
        for production in self.browse(cr, uid, ids):
            ...
            if not production.origin:
                message = _("Hello sample text for '%s'.") % (production.name)
